...
parallel-ssh -t 0 -i -P -H "$WORKER_IPS" -x "-o StrictHostKeyChecking=no -i /home/ubuntu/ray_bootstrap_key.pem" -I 'bash -s - '$HEAD_IP $USE_GCS_ONLY $GCS_DELAY_MS < $DIR/start_worker.sh
