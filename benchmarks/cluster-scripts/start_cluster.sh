...
NONDETERMINISM=${5:-0}
MAX_FAILURES=${6:-1}
MAX_LINEAGE_SIZE=${7:-0} # TODO fix (1 == flush disabled, !1 == flush enabled)
...
then
    ...
    echo "Usage: ./start_cluster.sh <num raylets> <num redis shards> <use gcs only> <GCS delay ms> <nondeterminism> <max_failures> <max lineage size>"
    exit
fi
...
bash $DIR/start_head.sh $NUM_REDIS_SHARDS $USE_GCS_ONLY $GCS_DELAY_MS $NONDETERMINISM $MAX_FAILURES $MAX_LINEAGE_SIZE $OBJECT_STORE_MEMORY_GB 
echo "Done starting head"

echo "Starting workers $WORKER_IPS with GCS delay $GCS_DELAY_MS and $NUM_RAYLETS raylets..."
parallel-ssh -t 0 -i -P -H "$WORKER_IPS" -x "-oStrictHostKeyChecking=no -i /home/ubuntu/ray_bootstrap_key.pem" -I 'bash -s - '$HEAD_IP $USE_GCS_ONLY $GCS_DELAY_MS $NONDETERMINISM $MAX_FAILURES $MAX_LINEAGE_SIZE $OBJECT_STORE_MEMORY_GB $OBJECT_STORE_EVICTION $PEG $OBJECT_MANAGER_THREADS < $DIR/start_worker.sh
