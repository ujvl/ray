...
void NodeManager::ProcessRegisterClientRequestMessage(
  ...
  auto worker =
      std::make_shared<Worker>(WorkerID::from_random(), message->worker_pid(), message->language(), client);
...
void NodeManager::SubmitTask(const Task &task, const Lineage &uncommitted_lineage,
  ...
  // TODO: Don't flush the task again on resubmission?
  if (use_gcs_only_) {
    if (!forwarded) {
...
void NodeManager::AssignTasksToWorker(const std::vector<Task> &tasks, std::shared_ptr<Worker> worker) {
  ...
  const auto assign_callback = [this, worker, task_ids, assigned_tasks](ray::Status status) {
    if (status.ok()) {
      // We successfully assigned the task to the worker.
      const TaskSpecification &first_spec = assigned_tasks.front().GetTaskSpecification();
