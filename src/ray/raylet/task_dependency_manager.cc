...
bool TaskDependencyManager::CheckObjectRequired(const ObjectID &object_id,
  ...
  if (delay_pull) {
    *delay_pull = it->second.delay_pull;
  }
  return true;
}
...
std::vector<TaskID> TaskDependencyManager::HandleObjectLocal(
  ...
  if (creating_task_entry != required_tasks_.end()) {
    auto object_entry = creating_task_entry->second.required_objects.find(object_id);
    if (object_entry != creating_task_entry->second.required_objects.end()) {
      ...
      if (object_entry->second.Empty()) {
        creating_task_entry->second.required_objects.erase(object_entry);
...
void TaskDependencyManager::HandleTaskResubmitted(
  ...
  for (auto required_object_it = required_objects.begin(); required_object_it
    ...
    // Remove the dependency from all workers that called `ray.wait` on the
    // newly available object.
    for (const auto &worker_id : required_object_it->second.dependent_workers) {
      RAY_CHECK(worker_dependencies_[worker_id].erase(required_object_it->first) > 0);
    }
    // Clear all workers that called `ray.wait` on this object, since the
    // `ray.wait` calls can now return the object as ready.
    required_object_it->second.dependent_workers.clear();
...
bool TaskDependencyManager::SubscribeGetDependencies(
    ...
    bool delay_pull, bool fast_reconstruction) {
  ...
  for (const auto &object_id : required_objects) {
    ...
    if (inserted.second) {
      ...
      required_task.required_objects[object_id].dependent_tasks.insert(task_id);
      if (delay_pull) {
        required_task.required_objects[object_id].delay_pull = true;
      }
      if (fast_reconstruction) {
  ...
  for (const auto &object_id : required_objects) {
    if (local_objects_.count(object_id) == 0) {
      ...
      if (inserted.second) {
        ...
        // dependent workers.
        required_tasks_[creating_task_id].required_objects[object_id].dependent_workers.insert(worker_id);
      }
    }
  }
...
bool TaskDependencyManager::UnsubscribeGetDependencies(const TaskID &task_id) {
  ...
  for (const auto &object_id : task_entry.get_dependencies) {
    ...
    // object.
    auto &dependent_tasks = creating_task_entry->second.required_objects[object_id].dependent_tasks;
    RAY_CHECK(dependent_tasks.erase(task_id) > 0);
    // If nothing else depends on the object, then erase the object entry.
    if (creating_task_entry->second.required_objects[object_id].Empty()) {
...
void TaskDependencyManager::UnsubscribeWaitDependencies(const WorkerID &worker_id) {
  ...
  for (const auto &object_id : worker_entry) {
    ...
    // object.
    auto &dependent_workers = creating_task_entry->second.required_objects[object_id].dependent_workers;
