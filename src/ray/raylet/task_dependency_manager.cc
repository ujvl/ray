...
bool TaskDependencyManager::CheckObjectRequired(const ObjectID &object_id, bool &fast_reconstruction) const {
  ...
  if (task_entry->second.dependent_tasks.count(object_id) == 0) {
    return false;
  }
...
std::vector<TaskID> TaskDependencyManager::HandleObjectLocal(
  ...
  if (creating_task_entry != required_tasks_.end()) {
    auto object_entry = creating_task_entry->second.dependent_tasks.find(object_id);
    if (object_entry != creating_task_entry->second.dependent_tasks.end()) {
...
bool TaskDependencyManager::SubscribeDependencies(
    const TaskID &task_id, const std::vector<ObjectID> &required_objects, bool fast_reconstruction) {
  ...
  for (const auto &object_id : required_objects) {
    ...
    if (inserted.second) {
      ...
      // dependent tasks.
      auto &dependent_tasks_entry = required_tasks_[creating_task_id];
      dependent_tasks_entry.dependent_tasks[object_id].push_back(task_id);
      if (fast_reconstruction) {
        dependent_tasks_entry.fast_reconstruction = true;
      }
    }
  }
...
void TaskDependencyManager::UnsubscribeDependencies(const TaskID &task_id) {
  ...
  for (const auto &object_id : task_entry.object_dependencies) {
    ...
    TaskID creating_task_id = ComputeTaskId(object_id);
    auto creating_task_entry = required_tasks_.find(creating_task_id);
    std::vector<TaskID> &dependent_tasks = creating_task_entry->second.dependent_tasks[object_id];
    ...
    if (dependent_tasks.empty()) {
      creating_task_entry->second.dependent_tasks.erase(object_id);
      // Remove the task that creates this object if there are no more object
      // dependencies created by the task.
      if (creating_task_entry->second.dependent_tasks.empty()) {
...
void TaskDependencyManager::TaskPending(const Task &task) {
  ...
  if (inserted.second) {
    ...
    if (remote_task_entry != required_tasks_.end()) {
      for (const auto &object_entry : remote_task_entry->second.dependent_tasks) {
