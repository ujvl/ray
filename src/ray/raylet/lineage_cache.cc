...
LineageCacheKFlush::LineageCacheKFlush(const ClientID &client_id,
                           gcs::TableInterface<TaskID, protocol::Task> &task_storage,
                           gcs::PubsubInterface<TaskID> &task_pubsub,
                           uint64_t max_lineage_size,
                           bool disabled)
...
/// \param stopping_condition A stopping condition for the DFS over
/// lineage_from. This should return true if the merge should stop.
void MergeLineageHelperKFlush(const TaskID &task_id, const Lineage &lineage_from,
                        Lineage &lineage_to,
                        std::function<bool(const LineageEntry &)> stopping_condition) {
  // If the entry is not found in the lineage to merge, then we stop since
  // there is nothing to copy into the merged lineage.
  ...
  if (lineage_to.SetEntry(entry->TaskData(), entry->GetStatus())) {
    for (const auto &parent_id : parent_ids) {
      MergeLineageHelperKFlush(parent_id, lineage_from, lineage_to, stopping_condition);
    }
  }
}

/// A helper function to merge one lineage into another, in DFS order.
void LineageCacheKFlush::AddUncommittedLineage(const TaskID &task_id, const Lineage &uncommitted_lineage) {
  // If the entry is not found in the lineage to merge, then we stop since
  // there is nothing to copy into the merged lineage.
...
bool LineageCacheKFlush::AddWaitingTask(const Task &task, const Lineage &uncommitted_lineage) {
  if (disabled_) {
    return true;
  }
  auto task_id = task.GetTaskSpecification().TaskId();
...
bool LineageCacheKFlush::AddReadyTask(const Task &task) {
  if (disabled_) {
    return true;
  }
  const TaskID task_id = task.GetTaskSpecification().TaskId();
...
bool LineageCacheKFlush::RemoveWaitingTask(const TaskID &task_id) {
  if (disabled_) {
    return true;
  }
  RAY_LOG(DEBUG) << "remove waiting task " << task_id << " on " << client_id_;
...
void LineageCacheKFlush::MarkTaskAsForwarded(const TaskID &task_id, const ClientID &node_id) {
  if (disabled_) {
    return;
  }
  RAY_CHECK(!node_id.is_nil());
...
void LineageCacheKFlush::FlushTask(const TaskID &task_id) {
  auto entry = lineage_.GetEntryMutable(task_id);
  RAY_CHECK(entry);
  RAY_CHECK(entry->GetStatus() == GcsStatus::UNCOMMITTED_READY);
...
bool LineageCacheKFlush::SubscribeTask(const TaskID &task_id) {
  auto inserted = subscribed_tasks_.insert(task_id);
  bool unsubscribed = inserted.second;
  if (unsubscribed) {
    // Request notifications for the task if we haven't already requested
...
bool LineageCacheKFlush::UnsubscribeTask(const TaskID &task_id) {
  auto it = subscribed_tasks_.find(task_id);
  bool subscribed = (it != subscribed_tasks_.end());
  if (subscribed) {
    // Cancel notifications for the task if we previously requested
...
void LineageCacheKFlush::EvictTask(const TaskID &task_id) {
...
void LineageCacheKFlush::EvictRemoteLineage(const TaskID &task_id) {
...
void LineageCacheKFlush::HandleEntryCommitted(const TaskID &task_id, bool lineage_committed) {
  if (disabled_) {
    return;
  }
  RAY_LOG(DEBUG) << "task committed: " << task_id;
...
const Task &LineageCacheKFlush::GetTask(const TaskID &task_id) const {
  const auto &entries = lineage_.GetEntries();
  auto it = entries.find(task_id);
  RAY_CHECK(it != entries.end());
  return it->second.TaskData();
...
bool LineageCacheKFlush::ContainsTask(const TaskID &task_id) const {
  const auto &entries = lineage_.GetEntries();
  auto it = entries.find(task_id);
  return it != entries.end();
}

size_t LineageCacheKFlush::NumEntries() const {
  return lineage_.GetEntries().size();
}

LineageCacheFlush::LineageCacheFlush(const ClientID &client_id,
                           gcs::TableInterface<TaskID, protocol::Task> &task_storage,
                           gcs::PubsubInterface<TaskID> &task_pubsub,
                           uint64_t max_lineage_size,
                           bool disabled)
...
/// \param stopping_condition A stopping condition for the DFS over
/// lineage_from. This should return true if the merge should stop.
void MergeLineageHelperFlush(const TaskID &task_id, const Lineage &lineage_from,
                        Lineage &lineage_to,
                        std::function<bool(const LineageEntry &)> stopping_condition) {
  // If the entry is not found in the lineage to merge, then we stop since
  // there is nothing to copy into the merged lineage.
  ...
  if (lineage_to.SetEntry(entry->TaskData(), entry->GetStatus())) {
    for (const auto &parent_id : parent_ids) {
      MergeLineageHelperFlush(parent_id, lineage_from, lineage_to, stopping_condition);
    }
  }
}

/// A helper function to merge one lineage into another, in DFS order.
void LineageCacheFlush::AddUncommittedLineage(const TaskID &task_id, const Lineage &uncommitted_lineage) {
  // If the entry is not found in the lineage to merge, then we stop since
  // there is nothing to copy into the merged lineage.
  auto entry = uncommitted_lineage.GetEntry(task_id);
  if (!entry) {
...
bool LineageCacheFlush::AddWaitingTask(const Task &task, const Lineage &uncommitted_lineage) {
  if (disabled_) {
    return true;
  }
  auto task_id = task.GetTaskSpecification().TaskId();
...
bool LineageCacheFlush::AddReadyTask(const Task &task) {
  if (disabled_) {
    return true;
  }
  const TaskID task_id = task.GetTaskSpecification().TaskId();
...
bool LineageCacheFlush::RemoveWaitingTask(const TaskID &task_id) {
  if (disabled_) {
    return true;
  }
  RAY_LOG(DEBUG) << "remove waiting task " << task_id << " on " << client_id_;
...
void LineageCacheFlush::MarkTaskAsForwarded(const TaskID &task_id, const ClientID &node_id) {
  if (disabled_) {
    return;
  }
  RAY_CHECK(!node_id.is_nil());
...
void LineageCacheFlush::FlushTask(const TaskID &task_id) {
  auto entry = lineage_.GetEntryMutable(task_id);
  RAY_CHECK(entry);
  RAY_CHECK(entry->GetStatus() == GcsStatus::UNCOMMITTED_READY);
...
bool LineageCacheFlush::SubscribeTask(const TaskID &task_id) {
  auto inserted = subscribed_tasks_.insert(task_id);
  bool unsubscribed = inserted.second;
  if (unsubscribed) {
    // Request notifications for the task if we haven't already requested
...
bool LineageCacheFlush::UnsubscribeTask(const TaskID &task_id) {
  auto it = subscribed_tasks_.find(task_id);
  bool subscribed = (it != subscribed_tasks_.end());
  if (subscribed) {
    // Cancel notifications for the task if we previously requested
...
void LineageCacheFlush::EvictTask(const TaskID &task_id) {
  auto commit_it = committed_tasks_.find(task_id);
  if (commit_it == committed_tasks_.end()) {
    return;
  }
...
void LineageCacheFlush::EvictRemoteLineage(const TaskID &task_id) {
  auto entry = lineage_.GetEntry(task_id);
  if (!entry) {
    return;
  }
...
void LineageCacheFlush::HandleEntryCommitted(const TaskID &task_id, bool evicted) {
  if (disabled_) {
    return;
  }
  RAY_LOG(DEBUG) << "task committed: " << task_id;
...
const Task &LineageCacheFlush::GetTask(const TaskID &task_id) const {
  const auto &entries = lineage_.GetEntries();
  auto it = entries.find(task_id);
  RAY_CHECK(it != entries.end());
  return it->second.TaskData();
...
bool LineageCacheFlush::ContainsTask(const TaskID &task_id) const {
  const auto &entries = lineage_.GetEntries();
  auto it = entries.find(task_id);
  return it != entries.end();
}

size_t LineageCacheFlush::NumEntries() const {
  return lineage_.GetEntries().size();
}
